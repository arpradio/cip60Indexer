...
1. **Core Indexer**: Connects to Cardano network via Ogmios- node connection is implied!  _(For full solution, we recommend their Docker container:[cardano-node-ogmios](https://hub.docker.com/r/cardanosolutions/cardano-node-ogmios))_, processes blocks, and extracts music token metadata
